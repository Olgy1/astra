...
export function RedditBlock({ config }: BlockProps<RedditBlockConfig>) {
  ...
  return (
    <a
      ...
      className="flex items-center gap-3 border border-[var(--card-border-color)] bg-[var(--card-bg)] p-4 transition-transform hover:scale-[1.01]"
...
export function DiscordServerBlock({ config, theme }: BlockProps<DiscordServerBlockConfig>) {
  ...
  const href = `https://discord.gg/${config.inviteCode}`;
  ...
  return (
    <a
      ...
      style={{ borderRadius: "var(--card-radius)" }}
    >
      <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#5865F2]">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium" style={{ fontFamily: blockFont }}>Serveur Discord</p>
        <p className="text-xs text-[var(--page-muted)]" style={{ fontFamily: blockFont }}>discord.gg/{config.inviteCode}</p>
      </div>
      <span className="shrink-0 rounded-lg bg-[#5865F2] px-3 py-1.5 text-xs font-medium text-white" style={{ fontFamily: blockFont }}>
        {config.buttonLabel}
      </span>
    </a>
  );
