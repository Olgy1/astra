...
import { pluralizeLabel, type VisitCounterBlockConfig } from "@/lib/blocks/definitions/visit-counter";
...
export function VisitCounterBlock({ config, page, theme }: BlockProps<VisitCounterBlockConfig>) {
  ...
  const content = (
    <>
      ...
      <span className="text-[var(--page-muted)]">{pluralizeLabel(config.label, display)}</span>
    </>
  );
